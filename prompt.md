...
用于以一张后台上传图片为主体的移动端页面。公开页面必须在图片上悬浮显示指向父页面的返回图标，不得渲染可见标题、背景装饰、正文、Card或页脚；后台上传时应等比缩放至最大720px宽并统一转换为PNG。
...
返回按钮必须始终打开数据库中记录的父页面URL，不得使用history.back()或其他浏览器历史跳转替代页面树导航，确保用户从二维码、微信、后台预览或任意页面进入时都能逐级返回首页。

除没有父页面的首页外，所有模板都必须显示返回图标，包括图片入口页、正文页、文字入口页和单张图片页。返回图标必须作为浮动层固定并尽量贴近视口右上安全区，页面滚动时保持原位且尽量不遮挡内容。“养老课堂”的每张图片入口必须在图片下方显示后台维护的入口标题。所有带页脚的页面统一显示“中国工商银行云南省分行 · 养老金融与资产托管部”。
...
- 设置内部页面链接
- 设置外部链接
- 入口编辑器将站内页面和“打开外部链接”统一放在“设置点击链接”下拉框中；只有选择外链时才显示并要求填写外部网址
- 入口目标默认只列出当前页面的直接下级页面，并提供默认开启的筛选开关；关闭后显示全部页面，编辑已有入口时不得因筛选而丢失原目标
- 查看生成后的页面
- 手动“重新生成全部网站”
- 显示生成成功或失败信息
