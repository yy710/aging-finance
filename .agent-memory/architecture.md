...
- `card-list` 的养老课堂布局为两列，网格宽度为页面画布的 72.2222%，并通过较小的黑体 `visual-card__title` 显示后台入口标题。
- 所有非首页模板都渲染 `partials/back-button.ejs`，其 `href` 来自数据库父页面 URL；共享 CSS 将按钮固定为视口右上角浮动层，并在宽屏上保持与 720px 页面画布的原有对齐。
